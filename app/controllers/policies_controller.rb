...
class PoliciesController < ApplicationController
  ...
  def new
    if logged_in?
      ...
      @pol_id = params[:id]
      @user = current_user
      redirect_to policies_select_company_path if @c_id.nil?
    else
      redirect_to login_path
    end
  end
  ...
  def policy_params
    params.require(:policy).permit(:policy_type, :description, :sum_insured, :premium_amt, :commission, :purchase_date,
                                   :mature_date, :user_id, :company_id, :policytype_id, documents: [])
  end
end
